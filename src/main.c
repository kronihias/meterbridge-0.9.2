...
int main(int argc, char *argv[])
{
	...
	int opt;
	float ref_lev;
	int port_base = 1; // something strange with command line args
    ...
    // added by Matthias Kronlachner
    int             opts;
    jack_status_t  stat;
    opts = JackNoStartServer;
    
	if ((client = jack_client_open (client_name, (jack_options_t) opts, &stat)) == 0) {
